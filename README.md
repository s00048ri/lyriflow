...
- **Setlist management** — Add multiple songs, reorder with drag & drop (touch supported), right-click to delete
- **Start from any song** — Select a song and press Perform to start from that point in the setlist
...
- **セットリスト管理** — 複数曲の登録、ドラッグ＆ドロップで並べ替え（タッチ対応）、右クリックで削除
- **任意の曲から開始** — 曲を選択してPerformを押すとその曲からスライドショーを開始
